...
<p align="center">
  <em>Individual Internship Project · Developed at Yonnov'IA</em>
</p>
...
I developed this project to an intelligent AI pipeline for processing scientific PDF documents, from document upload and extraction to preprocessing, automatic summarization, quality evaluation, fallback mechanisms, data persistence, and result export.
...
The solution combines conventional document-processing techniques with Large Language Models (LLMs) to improve the reliability and quality of the generated results.
...
I developed this project as part of an AI internship following the CRISP-DM methodology, with a focus on building a modular, robust, and extensible document-processing pipeline.

Project Objectives

My main objective is to transform scientific PDF documents into structured and exploitable information while providing automatic quality control at different stages of the pipeline.
...
4⃣ Extractive Summarization Fallback
...
My backend uses PostgreSQL to persist document information, pipeline execution history, preprocessing results, inference information, and generated summaries.

I use SQLAlchemy to access the database through SQLAlchemy.
...
My current implementation includes basic security and confidentiality controls for document processing.
...
I centralize the backend configuration through environment variables.
...
Qwen Vision / OCR-based processing

Extraction Quality Evaluation

Qwen Vision / LLM-as-a-Judge
...
git clone https://github.com/mohamedkarimmoalla-cmyk/pdf-ai-pipeline.git
cd pdf-ai-pipeline
...
My complete processing workflow follows these steps:

Upload a scientific PDF document.

Validate the file extension.

Verify that the uploaded file is readable.

Extract the document using PyMuPDF4LLM.

Evaluate extraction quality using the LLM-as-a-Judge.

If the score is below 75/100, switch to Docling.
...
Divide the selected content into chunks.

Check Qwen/Ollama availability.

Generate summaries for the chunks.

Merge the partial summaries into a global summary.

Evaluate the global summary using the Summary Judge.

Use the extractive fallback if LLM summarization is unavailable or fails.

Persist pipeline results in PostgreSQL.

Export the final results.

Current Results

My current implementation provides the following capabilities:
